...
const registerUser = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(201)
      .json({ message: "User profile created successfully", user: result[0] });
  } catch (error) {
    console.error(error);
    return res
      .status(500)
      .json({ message: "Failed to create user profile, Try again later." });
    throw error;
  }
};
...
const deleteUser = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "User profile deleted successfully", user: result[0] });
  } catch (error) {
    console.error(error);
    return res
      .status(500)
